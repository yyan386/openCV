...
class DecodeDetectionsFast(Layer):
    ...
    def __init__(self,
        ...
        if tf.keras.backend.backend() != 'tensorflow':
            raise TypeError("This layer only supports TensorFlow at the moment, but you are using the {} backend.".format(tf.keras.backend.backend()))
    ...
    def call(self, y_pred, mask=None):
        ...
        class_ids = channels_last.expand_dims(channels_last.to_float(channels_last.argmax(y_pred[...,:-12], axis=-1)), axis=-1)
        # Extract the confidences of the maximal classes.
        confidences = channels_last.reduce_max(y_pred[...,:-12], axis=-1, keep_dims=True)
        ...
        y_pred = channels_last.concat(values=[class_ids, confidences, xmin, ymin, xmax, ymax], axis=-1)
        ...
        def filter_predictions(batch_item):
            ...
            positive_boxes = channels_last.not_equal(batch_item[...,0], 0.0)
            predictions = channels_last.boolean_mask(tensor=batch_item,
                                          mask=positive_boxes)

            def perform_confidence_thresholding():
                # Apply confidence thresholding.
                threshold_met = predictions[:,1] > self.channels_last_confidence_thresh
                return channels_last.boolean_mask(tensor=predictions,
                                       mask=threshold_met)
            ...
            predictions_conf_thresh = channels_last.cond(channels_last.equal(channels_last.size(predictions), 0), no_positive_boxes, perform_confidence_thresholding)
            ...
            def perform_nms():
                ...
                xmin = channels_last.expand_dims(predictions_conf_thresh[...,-4], axis=-1)
                ymin = channels_last.expand_dims(predictions_conf_thresh[...,-3], axis=-1)
                xmax = channels_last.expand_dims(predictions_conf_thresh[...,-2], axis=-1)
                ymax = channels_last.expand_dims(predictions_conf_thresh[...,-1], axis=-1)
                ...
                maxima = channels_last.gather(params=predictions_conf_thresh,
            ...
            predictions_nms = channels_last.cond(channels_last.equal(channels_last.size(predictions_conf_thresh), 0), no_confident_predictions, perform_nms)
            ...
            def top_k():
                return channels_last.gather(params=predictions_nms,
                                 indices=channels_last.nn.top_k(predictions_nms[:, 1], k=self.channels_last_top_k, sorted=True).indices,
                                 axis=0)
            def pad_and_top_k():
                padded_predictions = channels_last.pad(tensor=predictions_nms,
                                            paddings=[[0, self.channels_last_top_k - channels_last.shape(predictions_nms)[0]], [0, 0]],
                                            mode='CONSTANT',
                                            constant_values=0.0)
            ...
            top_k_boxes = channels_last.cond(channels_last.greater_equal(channels_last.shape(predictions_nms)[0], self.channels_last_top_k), top_k, pad_and_top_k)
