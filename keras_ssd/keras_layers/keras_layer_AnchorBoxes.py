...
class AnchorBoxes(Layer):
    ...
    Input shape:
        ...
        or `(batch, height, width, channels)` if `dim_ordering = 'channels_last'`.
    ...
    def call(self, x, mask=None):
        ...
        Arguments:
            x (tensor): 4D tensor of shape `(batch, channels, height, width)` if `dim_ordering = 'th'`
                or `(batch, height, width, channels)` if `dim_ordering = 'channels_last'`. The input for this
        ...
        boxes_tensor = tf.keras.backend.tile(tf.keras.backend.constant(boxes_tensor, dtype='float32'), (tf.keras.backend.shape(x)[0], 1, 1, 1, 1))
